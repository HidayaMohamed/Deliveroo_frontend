...
export default function RiderProfile() {
  ...
  const openWhatsApp = () => {
    const msg = encodeURIComponent(
      `Rider Support, this is ${user?.full_name || user?.email}. I need assistance.`,
    );
          ...
          <div className="flex gap-4">
            <div className="px-8 py-4 bg-white rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                {user?.is_active ? "On Duty" : "Offline"}
              </p>
            </div>
          </div>
        </div>
              ...
              {[
                {
                  ...
                  val: stats?.total_deliveries ?? 0,
                  sub: `${stats?.completed_deliveries ?? 0} completed`,
                  icon: <Wallet className="text-emerald-500" />,
                  color: "emerald",
                },
                {
                  label: "Success Rate",
                  val: `${stats?.success_rate ?? "N/A"}%`,
                  sub: `${stats?.active_deliveries ?? 0} active now`,
                  icon: <TrendingUp className="text-blue-500" />,
                  color: "blue",
                },
                {
                  label: "Today",
                  val: stats?.todays_deliveries ?? 0,
                  sub: "deliveries today",
                  icon: (
