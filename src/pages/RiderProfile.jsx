...
export default function RiderProfile() {
        ...
        if (response.ok) {
          ...
          if (contentType && contentType.includes('application/json')) {
            ...
            setRiderData({
              ...
              email: data.email || "rider@deliveroo.ke",
              phone: data.phone || "+254 XXX XXX XXX",
              rating: data.rating || 4.8,
              tripsTotal: data.total_deliveries || 0,
              tripsThisMonth: data.monthly_deliveries || 0,
              ...
              plate_number: data.plate_number || "XXX-000",
            });
          } else {
            // Non-JSON response, use fallback data
  ...
  const openWhatsApp = () => {
    const msg = encodeURIComponent(
      `Rider Support, this is ${riderData?.name || "Rider"} (${riderData?.id || "RID-0000"}). I need assistance.`,
    );
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F1F5F9] py-12 px-[5%] font-sans flex items-center justify-center">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 mb-12">
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="px-8 py-4 bg-red-50 text-red-500 rounded-3xl shadow-sm border border-red-100 flex items-center gap-2 hover:bg-red-100 transition-colors"
            >
              <LogOut size={16} />
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                {
                  label: "Daily Revenue",
                  val: `KES ${riderData?.todayEarnings?.toLocaleString() || 0}`,
                  sub: "+12% vs Yesterday",
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div
                      className={`${stat.color === "yellow" ? "bg-slate-50" : "bg-white/10"} p-3 rounded-xl`}
                    >
                      {stat.icon}
                    </div>
                    <BarChart3
                  ...
                  <p
                    className={`text-4xl font-black italic tracking-tighter ${stat.color === "yellow" ? "text-slate-900" : "text-white"}`}
                  >
                    {stat.val}
                  </p>
                  <p
                    className={`text-[10px] font-black uppercase tracking-widest mt-1 ${stat.color === "yellow" ? "text-slate-400" : "text-white/40"}`}
                  >
                    {stat.label}
                  </p>
                </motion.div>
            ...
            <div className="bg-white p-10 rounded-[50px] shadow-sm border border-slate-100">
              <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-300 mb-8 italic">
                Operator Credentials
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div className="flex items-center gap-5">
                    <div className="p-4 bg-slate-50 rounded-2xl text-slate-900 shadow-inner">
                      <User size={20} />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-black text-slate-900">
                        {riderData?.vehicle_type || "N/A"} (
                        {riderData?.plate_number || "N/A"})
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900 rounded-[40px] p-8 flex flex-col justify-center border border-slate-100 text-white relative overflow-hidden">
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <p className="text-xs font-black uppercase tracking-widest italic text-yellow-500">
                        Fleet Verified
                      </p>
                    </div>
                    <p className="text-sm font-bold text-slate-400 leading-relaxed italic">
                      Account verified for{" "}
                      <span className="text-white">Commercial Logistics</span>{" "}
                      in Nairobi County.
                    </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                ...
                <p className="text-4xl font-black italic tracking-tighter text-slate-900">
                  {riderData?.tripsTotal?.toLocaleString() || 0}
                </p>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">
                  {riderData?.tripsThisMonth || 0} this month
                </p>
              </div>
              <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
                <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-2">
            ...
            <motion.div className="bg-yellow-400 p-10 rounded-[50px] flex flex-col md:flex-row items-center gap-8 justify-between shadow-2xl shadow-yellow-400/20">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-black/60 uppercase tracking-widest mt-2">
                    Activate GPS tracking & Mission Feed
                  </p>
                </div>
              </div>
              <motion.button
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-slate-900 p-10 rounded-[50px] shadow-2xl relative overflow-hidden group">
              ...
              <h4 className="text-5xl font-black text-white tracking-tighter italic">
                <span className="text-yellow-400 text-xl mr-2">KES</span>
                {(riderData?.walletBalance || 0).toLocaleString()}
              </h4>
              <button className="mt-4 px-6 py-3 bg-white/10 text-white rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-white/20 transition-colors w-full">
                Withdraw Funds
              </button>
            </div>
            ...
            <div className="bg-white p-10 rounded-[50px] shadow-sm border border-slate-100">
              ...
              <div className="space-y-4">
                ...
                <button
                  onClick={() => navigate("/courier/dashboard")}
                  className="w-full py-5 bg-yellow-400 text-black rounded-[25px] font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 hover:bg-yellow-300 transition-colors"
                >
                  <Truck size={18} /> Back to Dashboard
                </button>
              </div>
            </div>
          </div>
